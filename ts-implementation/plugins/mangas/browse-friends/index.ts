...
const ROUTE = '/mangas/friends';

type BrowseFriendsData = {
	works: WorkSummary[];
	error: string | null;
};
...
export default definePlugin<MugenHostApi>({
	manifest: {
		id: 'org.mugens.core.mangas.friends',
		name: 'Mangas Friends',
		version: '2.0.0',
	...
	routes: [
		route<MugenHostApi>(ROUTE)
			.load<BrowseFriendsData>(async ({ api }) => {
			...
			.render((data, renderCtx) => {
				...
				return buildBrowseScreen({
					...
					works: data.works,
					error: data.error,
					emptyText: t('catalog_empty_works'),
					activeTab: 'friends'
				});
			})
	]
