...
const ROUTE = '/mangas/recent';

type BrowseRecentData = {
	works: WorkSummary[];
	error: string | null;
};
...
export default definePlugin<MugenHostApi>({
	manifest: {
		id: 'org.mugens.core.mangas.recent',
		name: 'Mangas Recent',
		version: '2.0.0',
	...
	routes: [
		route<MugenHostApi>(ROUTE)
			.load<BrowseRecentData>(async ({ api }) => {
			...
			.render((data, renderCtx) => {
				...
				return buildBrowseScreen({
					...
					works: data.works,
					error: data.error,
					emptyText: t('catalog_empty_recent'),
					activeTab: 'recent'
				});
			})
	]
