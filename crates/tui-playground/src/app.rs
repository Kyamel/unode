//! The playground application: shell state (route, focus, panes), the plugin
//! session lifecycle, and the render/dispatch loop. Rendering appearance
//! lives in `renderer.rs`; plugin discovery in `plugin_registry.rs`; the
//! shell's own routes in `shell_registry.rs`.
...
use unode_ratatui_renderer::{
    ...
    collect_screen_interactions, render_tui_shell,
};
use unode_runtime::{CommandResult, ShellContext};
use unode_tui_runtime::{PluginSession, TuiRuntime, resolve_screen_state};

use crate::plugin_registry::{LoadedPlugin, load_runtime_plugins};
use crate::renderer::playground_renderer;
use crate::route::parse_route;
use crate::shell_registry::register_builtin_shell;
...
pub(crate) struct App {
    ...
    pub(crate) current_route: String,
    pub(crate) selected_nav: usize,
    pub(crate) command_mode: bool,
    pub(crate) command_input: String,
    pub(crate) status: String,
    pub(crate) main_panel: TuiMainContent,
    pub(crate) focused_pane: TuiFocusedPane,
    pub(crate) main_interactions: Vec<TuiInteractiveElement>,
    pub(crate) selected_main_interaction: Option<usize>,
    pub(crate) plugins: Vec<LoadedPlugin>,
    pub(crate) active_plugin_session: Option<ActivePluginSession>,
}
...
impl App {
    pub(crate) fn new() -> Result<Self> {
        ...
        let renderer = playground_renderer();
    ...
    pub(crate) fn navigate_to(&mut self, route: String) {
    ...
    pub(crate) fn activate_main_interaction(&mut self) -> Result<()> {
