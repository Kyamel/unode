...
pub fn read_bytes(memory: &[u8], ptr: u32, len: u32) -> Result<Vec<u8>, WebMemoryError> {
    let start = ptr as usize;
    let count = len as usize;
    let end = start
        .checked_add(count)
        .ok_or(WebMemoryError::OutOfBounds {
            ptr: start,
            len: count,
    ...
    if end > memory.len() {
        return Err(WebMemoryError::OutOfBounds {
            ptr: start,
            len: count,
            memory_len: memory.len(),
        });
...
pub fn read_json<T: DeserializeOwned>(memory: &[u8], ptr: u32, len: u32) -> Result<T, WebMemoryError> {
    let bytes = read_bytes(memory, ptr, len)?;
    decode_json_bytes(&bytes).map_err(WebMemoryError::from)
}

pub fn write_bytes(memory: &mut Vec<u8>, ptr: u32, bytes: &[u8]) -> Result<(), WebMemoryError> {
    let start = ptr as usize;
    let end = start
...
pub fn write_json<T: Serialize>(memory: &mut Vec<u8>, ptr: u32, value: &T) -> Result<usize, WebMemoryError> {
...
mod tests {
    ...
    use super::{read_bytes, read_json, write_bytes, write_json, WebMemoryError};
    ...
    fn errors_on_out_of_bounds_reads() {
        ...
        assert!(matches!(
            ...
            Err(WebMemoryError::OutOfBounds { .. })
        ));
    }
}
