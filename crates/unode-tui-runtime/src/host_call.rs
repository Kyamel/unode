...
pub struct TuiHostCallDispatcher {
    handlers: BTreeMap<String, HostCallHandler>,
    last_response: Vec<u8>,
}

impl std::fmt::Debug for TuiHostCallDispatcher {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TuiHostCallDispatcher")
...
impl TuiHostCallDispatcher {
    ...
    pub fn dispatch_envelope(
        ...
        let handler = self
            ...
            .ok_or_else(|| TuiHostCallError::UnknownOperation(envelope.operation.clone()))?;

        let result = handler(&envelope.params).map_err(|err| match err {
            TuiHostCallError::Handler { .. } => err,
            other => TuiHostCallError::Handler {
    ...
    pub fn dispatch_bytes(&mut self, request_bytes: &[u8]) -> Result<&[u8], TuiHostCallError> {
...
mod tests {
    ...
    fn errors_on_unknown_operation() {
        ...
        assert!(matches!(
            ...
            Err(TuiHostCallError::UnknownOperation(op)) if op == "missing.op"
        ));
    }
}
