...
Both web packages use the same Rust/WASM core boundary:
...
| `web-react` (`unode-react`) | React adapter using `useSyncExternalStore` for per-node subscriptions and `createReactRenderer()` for app-defined visual mapping. |
| `web-svelte` (`unode-svelte`) | Svelte adapter using `createSubscriber` for per-node subscriptions. |
