//! `complex-layout` — one capability: **the layout node vocabulary**.
//!
//! A dashboard built only from semantic containers (`section`, `stack`,
//! `inline`, `grid`, `list`, `value`, `badge`, `divider`...) with zero
//! styling information. The lesson: the same `ScreenNode` tree is mounted by
//! React, Svelte, Vue, Solid, or a terminal renderer — layout intent
//! survives, presentation is the renderer's.
...
const PLUGIN_ID: &str = "dev.unode.layout";
const PLUGIN_NAME: &str = "Layout";
...
fn render_screen(_request: &PluginRenderRequest) -> ScreenNode {
    ui::screen()
        .id("layout.screen")
        .title(PLUGIN_NAME)
        .subtitle("One capability: the layout vocabulary. Pure semantic containers, zero styling info.")
        .children(ui::nodes![
            ui::grid()
                .id("layout.metrics")
                .max_columns(4)
