//! `route-tabs` — one capability: **route groups with a tabs intent**.
//!
//! The manifest declares three routes in one group (`tabs()`); the host
//! derives the tab bar from it — the active tab comes from the matched
//! route (it can never drift) and the Ship badge is a state binding, so a
//! dispatch that writes `routeTabs.shipCount` updates chrome the plugin
//! never rendered. Intent is a hint: hosts without tabs treat the routes as
//! plain screens. For dispatch outcomes (navigate/refresh), see `actions`.
...
use unode_plugin_sdk::prelude::{
    ...
    PluginManifestEnvelope, PluginRenderRequest, ScreenNode, StateKey, TextRole, Tone, builtin,
    perm, route_group,
};

const PLUGIN_ID: &str = "dev.unode.route-tabs";
const PLUGIN_NAME: &str = "Route Tabs";
const COMPOSE_PATH: &str = "/plugins/route-tabs";
const REVIEW_PATH: &str = "/plugins/route-tabs/review";
const SHIP_PATH: &str = "/plugins/route-tabs/ship";
...
fn render_screen(request: &PluginRenderRequest) -> ScreenNode {
    ...
    ui::screen()
        ...
        .initial_focus("route-tabs.compose")
        .build()
}

/// Increments the ship counter and records the last-ship message. The badge
/// binding in the manifest and the text binding on the screen both pick the
/// writes up — chrome and content update from the same state.
