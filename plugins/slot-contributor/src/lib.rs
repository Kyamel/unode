//! `split-contributor` — one capability: **contributing to slots** (the
//! guest side of cross-plugin UI).
//!
//! The manifest declares slot contributions targeting `split-host`'s named
//! slots; `plugin_render_slot` returns the nodes to inject. The trust
//! boundary is the lesson: injected actions keep their contributor origin,
//! so clicks dispatch back to THIS plugin, never to the host plugin.
//! Pair with `split-host`.
...
const PLUGIN_ID: &str = "dev.unode.slot-contributor";
const PLUGIN_NAME: &str = "Slot Contributor";
...
fn manifest_envelope() -> PluginManifestEnvelope {
    ui::plugin_manifest(PLUGIN_ID, PLUGIN_NAME)
        ...
        .author("unode")
        .permission(
            perm("slot.contribute:slot-host")
                .required(true)
        ...
        .slot_contributions([
            SlotContributionDecl {
                ...
                target: "slot-host:right".to_string(),
                priority: 100,
                when: None,
            },
            SlotContributionDecl {
                id: "footer-status".to_string(),
                target: "slot-host:footer".to_string(),
                priority: 50,
                when: None,
            },
        ])
...
fn render_screen(_request: &PluginRenderRequest) -> ScreenNode {
    ui::screen()
        .id("slot-contributor.screen")
        .title(PLUGIN_NAME)
        .subtitle("One capability: contributing to slots. Open Split Screen Host to see this plugin inside another.")
        .children(ui::nodes![
            ui::text("This plugin declares slot contributions in its manifest.")
                .role(TextRole::Body)
                .tone(Tone::Info),
            ui::text("Trust boundary: when injected, the button keeps its contributor origin — clicks dispatch to THIS plugin, never to the host plugin.")
...
mod tests {
    ...
    fn manifest_declares_slot_contributions() {
        ...
        assert_eq!(manifest.slot_contributions[0].target, "slot-host:right");
    }
