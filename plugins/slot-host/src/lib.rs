//! `split-host` — one capability: **owning slots** (the host side of
//! cross-plugin UI).
//!
//! This plugin renders a layout with named `slot` nodes
//! (`slot-host:right`, `slot-host:footer`). It never imports
//! the contributor: the host runtime finds manifest-declared contributions,
//! calls the other plugin's `plugin_render_slot`, and mounts the returned
//! nodes into these holes. Pair with `split-contributor`.
...
const PLUGIN_ID: &str = "dev.unode.slot-host";
const PLUGIN_NAME: &str = "Slot Host";
...
fn render_screen(_request: &PluginRenderRequest) -> ScreenNode {
    ui::screen()
        ...
        .title(PLUGIN_NAME)
        .subtitle(
            "One capability: owning slots. This plugin renders holes; another plugin fills them.",
        )
        ...
        .children(ui::nodes![
            ui::grid()
                ...
                .children(ui::nodes![
                    ...
                    ui::section()
                        ...
                        .children(ui::nodes![
                            ui::slot("slot-host:right")
                                .id("slot-host.right-slot")
                                .fallback(
                                    ui::text("No contributor responded to slot-host:right.")
            ...
            ui::section()
                ...
                .title("Footer slot")
                .children(ui::nodes![
                    ui::slot("slot-host:footer")
                        .id("slot-host.footer-slot")
                        .fallback(ui::text("Footer slot fallback.").tone(Tone::Muted)),
                ])
                .into_node(),
        ])
        .initial_focus("slot-host.refresh")
        .build()
}
...
mod tests {
    ...
    fn render_contains_slot_anchors() {
        ...
        .unwrap();
        assert!(encoded.contains("slot-host:right"));
        assert!(encoded.contains("slot-host:footer"));
    }
}
