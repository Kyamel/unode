//! `complex-state` — one capability: **collections driven by state**.
//!
//! Where `counter` binds a single text line, this board binds several labels
//! and a list whose items carry per-item actions. One dispatch writes
//! multiple state paths; the host's resolver finds every bound node and
//! patches exactly those. The lesson: reactivity granularity — no full
//! re-render, one patch per affected node.
...
const PLUGIN_ID: &str = "dev.unode.state-collections";
const PLUGIN_NAME: &str = "State Collections";
...
fn render_screen(request: &PluginRenderRequest) -> ScreenNode {
    ...
    ui::screen()
        .id("state-collections.screen")
        .title(PLUGIN_NAME)
        .subtitle("One capability: collections driven by state. Many bindings, one patch per affected node.")
        .initial_state(initial)
        .children(ui::nodes![
            ui::grid()
                .id("state-collections.metrics")
                .max_columns(3)
                .children(ui::nodes![
                    ui::text(expr::binding::<String>("board.doneLabel"))
                        .id("state-collections.done")
                        .role(TextRole::Title)
                        .tone(Tone::Success),
                    ui::text(expr::binding::<String>("board.doingLabel"))
                        .id("state-collections.doing")
                        .role(TextRole::Title)
                        .tone(Tone::Warning),
                    ui::text(expr::binding::<String>("board.todoLabel"))
                        .id("state-collections.todo")
                        .role(TextRole::Title)
                        .tone(Tone::Info),
                ]),
            ui::section()
                .id("state-collections.board")
                .title("Task board")
                .description("Each action writes a small set of derived host-state paths.")
                .children(ui::nodes![
                    ui::list(items),
                    ui::actions()
                        .id("state-collections.actions")
                        .children([
                            ui::action("Advance board", custom("board.advance"))
                                .id("state-collections.advance")
                                .intent(ActionIntent::Primary),
                            ui::action("Reset board", custom("board.reset"))
                                .id("state-collections.reset")
                                .intent(ActionIntent::Ghost),
                        ]),
                ]),
        ])
        .initial_focus("state-collections.advance")
        .build()
}
