...
adapter. **React and Svelte** are the maintained adapters; they consume the IR,
keep a keyed `ScreenStore`, and apply patch ops by node key.

**TUI renderer** — native Rust. It instantiates the plugin `.wasm` via Wasmtime,
implements host functions as Rust closures, receives `CanonicalScreen` JSON, and
drives Ratatui + `ratatui-image` for rendering.
...
9.  Host tracks reactive bindings
10. Host lowers the canonical screen to IR
11. Renderer adapter mounts the IR
12. User interaction → ActionRef dispatched → plugin action handler runs (WASM)
13. Handler requests state writes → crosses the WASM boundary → StateStore updates
14. Writes are planned into patch ops → only affected nodes are patched
15. Navigation → tear down subscriptions, reset StateStore, repeat from step 5
```

Steps 1–4 happen once per plugin. Steps 5–11 happen on each navigation. Steps
12–14 happen on each interaction. Step 15 restarts the cycle.
