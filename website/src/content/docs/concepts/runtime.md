...
A plugin is a WASM module that exports four lifecycle entry points:
...
10. Teardown on navigation
    ...
    // the WASM instance may be pooled for reuse
```
...
WASM instances are expensive to parse, compile, and link, so the runtime keeps a
pool of instantiated modules and resets them between uses. Only the StateStore
is recreated per navigation — the WASM module itself is reused.
