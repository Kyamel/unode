...
**Trusted host runtime as security boundary.** The host runtime owns plugin
loading, sandboxing, capability injection, permission enforcement, resource
policy, and crash isolation. Renderers own presentation, focus, keyboard
behavior, and platform-specific accessibility. The plugin only declares intent.
...
- Owns web-side resource, state, and plugin-fault policy as those contracts land
...
**TUI runtime + Ratatui renderer (Rust)**
...
- Treats mounted plugin sessions as disposable isolation units
...
10. Host calls trackReactiveBindings(screen, resolver, state, on_patch)
11. Host lowers the canonical screen to IR
12. Renderer adapter mounts the IR
13. User interaction -> ActionRef dispatched -> plugin action handler called (WASM)
14. Action handler calls ctx.state.set() -> crosses WASM boundary -> StateStore updates
15. StateStore writes are planned into patch ops -> renderer patches only affected nodes
16. Navigation -> teardown subscriptions, reset StateStore, repeat from step 5
```

Steps 1–4 happen once per plugin activation/cache policy. Steps 5–12 happen on
each navigation. Steps 13–15 happen on each user interaction. Step 16 restarts
the cycle.
