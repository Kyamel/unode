...
// framework updates scoped to a single node key — the web analogue of the TUI
// patching only the affected terminal cells.
//
// Each keyed node component subscribes to its own key. A `SetProp` patch bumps
// only that key's version, so only that one component updates. `render()` is
// never re-run; the AST structure is fixed for the mount.

import { FIELD_CODE_TO_PROP, nodeKey } from "./ir";
import type { IrNode, IrPatchOp, IrScreen } from "./ir";
