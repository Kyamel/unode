...
//
// Build that module with:
//   wasm-pack build crates/unode-web-host --target web --out-dir <pkg>
// then point `loadWebHost` at the generated `unode_web_host.js`.
//
...
import type { IrPatchOp, IrScreen } from "./ir";
...
export class HostSession {
  ...
  setRoute(route: { pattern: string; params?: Record<string, string>; query?: Record<string, string> }): void {
  ...
  /** Patch ops that resolve every binding against the seeded state (apply once
   * right after mount — the mounted IR keeps bindings symbolic). */
